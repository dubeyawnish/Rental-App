...
function RentalDetails() {
  ...
  const makeProperyAvailable = async () => {
    ...
    const deleteTenant = await axios.delete(`${API_BASE_URL}/deleteTenant/${tenantId}`)

    const request = { isRented: false }
    const result = await axios.put(`${API_BASE_URL}/myTenants/${property._id}`, request, CONFIG_OBJ)
  ...
  const getTenantDetail = async (userId) => {
    const res = await fetch(`${API_BASE_URL}/myTenants/${userId}`, CONFIG_OBJ);
