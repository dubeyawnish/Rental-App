...
function EditUser() {
    ...
    const editUser = (event) => { 
        ...
        .then((data)=>{
            ...
            const request = { firstName, lastName, phone, addressLineOne, addressLineTwo, city, state, zipCode, country, imgName: data.data.fileName };
            debugger;
            axios.put(`${API_BASE_URL}/user/profile/${userId}`, request, CONFIG_OBJ)
            .then((data) => {
                if (data) {
                    setLoading(false);
                    Swal.fire({
